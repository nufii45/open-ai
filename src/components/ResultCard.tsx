...
export function ResultCard({ result, isSaved, onSave }: ResultCardProps) {
  ...
  const priceCheckedOn = comparison.checkedOn
  ...
  return (
    <article
      ...
      className="overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-[0_20px_60px_-30px_rgba(15,23,42,0.35)]"
    >
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 px-5 py-4 sm:px-6">
        ...
        <div className="flex items-center gap-2 text-xs text-slate-500">
          ...
          Verified comparison
          {isEstimated ? (
            <Badge className="bg-amber-50 text-amber-800">Estimated match</Badge>
          ) : null}
        </div>
      </div>
      ...
      <div className="px-5 pt-5 sm:px-6">
        <div
          ...
          role="tablist"
          aria-label="Medicine result sections"
        >
      ...
      <div
        id={panelId}
        role="tabpanel"
        aria-labelledby={`${comparison.id}-${activeTab}-tab`}
      ...
      >
          ...
          <div className="space-y-6">
            <div className="rounded-2xl bg-emerald-50 px-4 py-5 text-center sm:px-6 sm:py-6">
              <p className="text-sm font-semibold text-emerald-800">
                Potential saving per matching pack
              </p>
            ...
            <div>
              ...
              <p className="mt-1 text-sm leading-5 text-slate-600">
                ...
                <span className="font-medium text-slate-800">{comparison.activeIngredient}</span>
              </p>
            </div>
              ...
              <div className="flex items-start justify-between gap-4 p-4">
                <div className="min-w-0">
                  <dt className="text-xs font-medium text-slate-600">
                    Branded price · {comparison.brand}
                  </dt>
                  <dd className="mt-1.5 text-xs leading-5 text-slate-500">
                    {comparison.brandedPriceSource}
                  </dd>
                </div>
              ...
              <div className="flex items-start justify-between gap-4 bg-emerald-50/60 p-4">
                <div className="min-w-0">
                  <dt className="text-xs font-medium text-emerald-800">
                    Generic price · {comparison.generic}
                  </dt>
                  <dd className="mt-1.5 text-xs leading-5 text-emerald-800/80">
                    {comparison.genericPriceSource}
                  </dd>
                </div>
            ...
            >
              <p className="text-xs font-semibold uppercase tracking-wide text-slate-600">
                Evidence for this matching pack
              </p>
              <div className="mt-3 grid gap-2 text-sm text-slate-700 sm:grid-cols-2">
                <p>
                  <span className="font-medium text-slate-900">Brand source:</span>{' '}
                  {comparison.brandedEvidence.sourceName}
                </p>
                <p>
                  <span className="font-medium text-slate-900">Generic source:</span>{' '}
                  {comparison.genericEvidence.sourceName}
                </p>
              </div>
              <p className="mt-3 text-xs leading-5 text-slate-600">
                Same active ingredient, strength, dosage form, and pack.{' '}
                {priceCheckedOn
                  ? `Checked on ${priceCheckedOn}.`
                  : 'Current evidence date unavailable.'}
              </p>
            </section>
            ...
            <div className="border-t border-slate-200 pt-4">
              ...
              <p className="mt-3 text-xs text-slate-500">
                {priceCheckedOn
                  ? `Prices checked on ${priceCheckedOn}.`
                  : 'Price evidence date unavailable.'}
              </p>
            </div>
        ...
        {activeTab === 'impact' ? (
          <section aria-labelledby="personal-savings-heading" className="space-y-5">
            <div>
              ...
              <h3
                id="personal-savings-heading"
                className="mt-1 text-2xl font-bold tracking-tight text-slate-950"
              >
                Turn one pack into a yearly impact.
              </h3>
              <p className="mt-2 text-sm leading-6 text-slate-600">
                Use the number of matching packs you already buy each month.
              </p>
            </div>
            <label className="flex items-center justify-between gap-3 rounded-xl border border-teal-200 bg-teal-50/60 p-4 text-sm font-medium text-teal-950">
              ...
              <input
                ...
                onChange={(event) => {
                  ...
                  setPacksPerMonth(
                    Number.isFinite(value) ? Math.min(31, Math.max(1, Math.round(value))) : 1,
                  );
                }}
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <div className="rounded-2xl bg-slate-950 p-5 text-white">
                <p className="text-sm text-slate-300">Potential monthly saving</p>
                <p className="mt-2 text-3xl font-bold tabular-nums">{formatPHP(monthlySavings)}</p>
              </div>
              <div className="rounded-2xl bg-teal-600 p-5 text-white">
                <p className="text-sm text-teal-50">Potential yearly saving</p>
                <p className="mt-2 text-3xl font-bold tabular-nums">{formatPHP(annualSavings)}</p>
              </div>
            </div>
            <p className="text-xs leading-5 text-slate-500">
              Based on the packs you already purchase. This is not a dosage or treatment
              recommendation.
            </p>
          </section>
        ) : null}
        ...
        {activeTab === 'pharmacies' ? (
          <PharmacyLocator pharmacies={NEAREST_SAMPLE_PHARMACIES} category="pharmacy" />
        ) : null}
      </div>
      ...
      <div className="border-t border-slate-100 bg-slate-50/70 p-5 sm:px-6">
        <Button
          ...
          className="h-12 w-full bg-teal-600 text-white hover:bg-teal-700 disabled:bg-slate-200 disabled:text-slate-600"
        >
          {isSaved ? (
            <>
            ...
            <>
              <Plus aria-hidden="true" className="size-4" /> Save verified comparison
            </>
          )}
        </Button>
      </div>
