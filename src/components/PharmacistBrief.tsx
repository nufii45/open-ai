...
import {
  ...
  MessageCircleQuestion as Sparkles,
} from 'lucide-react';
...
export function PharmacistBrief({ comparisonId }: { comparisonId: string }) {
  ...
  async function prepareBrief() {
    ...
    try {
      ...
      const value = payload as {
        status?: string;
        source?: 'gpt-5.6-terra' | 'template';
        brief?: PharmacistBriefContent;
      };
      if (value.status !== 'ready' || !value.brief || !value.source)
        throw new Error('Brief unavailable');
  ...
  async function copyBrief() {
    ...
    const text = [
      state.brief.summary,
      state.brief.pharmacistQuestion,
      ...state.brief.checklist,
      state.brief.safetyReminder,
    ].join('\n');
    try {
  ...
  return (
    <section
      aria-labelledby="pharmacist-brief-heading"
    ...
    >
      <div className="flex items-start gap-3">
        ...
        <div>
          ...
          <h3
            id="pharmacist-brief-heading"
            className="mt-0.5 text-lg font-bold tracking-tight text-slate-950"
          >
            Prepare a pharmacist-ready question
          </h3>
          <p className="mt-1 text-sm leading-6 text-slate-700">
            Turn this verified comparison into a short checklist for your pharmacist. No symptoms or
        ...
        <div className="mt-4">
          ...
          {state.status === 'error' ? (
            <p role="status" className="mt-2 text-xs text-slate-600">
              The brief is unavailable right now. Please use the comparison checklist above when
        ...
        <div className="mt-4 rounded-xl border border-teal-200 bg-white p-4">
          <div className="flex items-center justify-between gap-3">
            <p className="text-xs font-semibold uppercase tracking-wide text-teal-800">
              {state.source === 'gpt-5.6-terra'
                ? 'GPT-5.6 assisted brief'
          ...
          <p className="mt-3 rounded-lg bg-slate-50 p-3 text-sm font-medium leading-6 text-slate-950">
            “{state.brief.pharmacistQuestion}”
          </p>
          <ul className="mt-3 space-y-1.5 text-sm text-slate-700">
            {state.brief.checklist.map((item) => (
