    ...
    <div className="flex flex-col items-center gap-3 rounded-2xl border border-slate-200 bg-white p-5 text-center shadow-sm sm:p-6">
      ...
      <p className="text-base font-semibold text-slate-950">
        {reason === 'draft_evidence'
          ? 'Price evidence is being checked'
          : 'Not yet verified locally'}
      </p>
      <p className="max-w-md text-sm leading-6 text-slate-600">
        {reason === 'draft_evidence' && query ? (
          <>
            We recognize <span className="font-medium text-slate-900">&ldquo;{query}&rdquo;</span>,
            but we will not show a savings claim until both matching packs have fresh local price
            evidence.
          </>
        ) : query ? (
          <>
      ...
      <p className="max-w-md text-sm leading-6 text-slate-600">
        ...
        HealthBridge does not recommend changing medicine on its own.
      </p>
    </div>
  );
}
