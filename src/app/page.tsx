      ...
      <div className="mx-auto w-full max-w-6xl">
        <header className="mb-8 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div><p className="text-lg font-bold tracking-tight">HealthBridge</p><p className="text-xs text-slate-500">Verified medicine comparisons</p></div>
          </div>
        ...
        {isLanding ? (
          <div className="grid gap-6 lg:grid-cols-[minmax(0,1fr)_20rem] lg:items-start">
            ...
            <div className="space-y-4">
              {trustPoints.map(({ icon: Icon, title, text }) => <div key={title} className="rounded-2xl border border-slate-200 bg-white p-5 shadow-sm"><Icon className="size-5 text-teal-700" aria-hidden="true" /><h2 className="mt-4 font-semibold tracking-tight">{title}</h2><p className="mt-1 text-sm leading-6 text-slate-600">{text}</p></div>)}
