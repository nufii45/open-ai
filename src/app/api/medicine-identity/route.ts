...
function readMatches(payload: unknown): IdentityMatch[] {
  const results = payload && typeof payload === 'object' && Array.isArray((payload as { results?: unknown }).results) ? (payload as { results: unknown[] }).results : [];
  const seen = new Set<string>();
  return results.flatMap((item) => {
    const openfda = item && typeof item === 'object' && (item as { openfda?: unknown }).openfda && typeof (item as { openfda?: unknown }).openfda === 'object' ? (item as { openfda: Record<string, unknown> }).openfda : null;
    if (!openfda) return [];
    const match: IdentityMatch = { brand: firstString(openfda.brand_name), generic: firstString(openfda.generic_name), dosageForm: firstString(openfda.dosage_form), manufacturer: firstString(openfda.manufacturer_name) };
    if (!match.brand && !match.generic) return [];
    const key = `${match.brand ?? ''}|${match.generic ?? ''}|${match.dosageForm ?? ''}`.toLowerCase();
...
export async function GET(request: NextRequest) {
  ...
  return NextResponse.json({
    ...
    notice: 'Possible US-label identities only. Confirm the ingredient, strength, form, and pack on Philippine packaging with a pharmacist. HealthBridge does not use these results for prices or switching advice.',
  });
}
