...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const entries = (data.features ?? []).flatMap((feature): Array<{ pharmacy: SamplePharmacy; location: PharmacyMapLocation }> => {
      ...
      if (!placeId || !name || typeof latitude !== 'number' || typeof longitude !== 'number' || !Number.isFinite(latitude) || !Number.isFinite(longitude) || seen.has(placeId)) return [];
      seen.add(placeId);
      const id = `geoapify-${placeId}`;
      const branch = feature.properties?.address_line1?.trim() || feature.properties?.formatted?.trim() || 'Address unavailable';
      return [{
        pharmacy: { id, name, branch, directionsUrl: directionsUrl(latitude, longitude), label: 'Nearby map result', note: 'Nearby map result — confirm stock, price, and operating hours directly with the pharmacy.' },
    ...
    return NextResponse.json({ error: 'Nearby pharmacy search is unavailable right now.' }, { status: 502 });
  }
}
