...
describe('visit note', () => {
  it('normalises a short user-authored visit note', () => {
    expect(normaliseVisitNote('  I have had a cough for two days.  ')).toBe(
  ...
  it('rejects model language that diagnoses or prescribes', () => {
    expect(
      parseVisitNote({
        summary: 'You likely have an infection.',
        questions: ['What should I ask today?', 'What should I bring?', 'Who can I call later?'],
      }),
    ).toBeNull();
    expect(
      resolveVisitNote('I have a concern to discuss.', 'clinic', {
