...
const matched = { strength: comparison.strength, dosageForm: comparison.dosageForm, packQuantity: comparison.packQuantity, brandedPrice: 50, genericPrice: 20, confirmedSamePack: true };
...
describe('medicine companion', () => {
  it('accepts a matching observed pack and calculates observed savings', () => {
    ...
    expect(calculateObservedImpact(matched, match)).toEqual({ savings: 30, percent: 60, status: 'saves' });
  });

  it('rejects a strength, form, pack, or confirmation mismatch', () => {
    const match = evaluatePackMatch(comparison, { ...matched, strength: '250 mg', dosageForm: 'Capsule', packQuantity: 20, confirmedSamePack: false });
  ...
  it('does not turn a higher observed generic price into a saving', () => {
    ...
    expect(calculateObservedImpact({ ...matched, genericPrice: 60 }, match)).toEqual({ savings: -10, percent: 20, status: 'no_saving' });
  });
});
